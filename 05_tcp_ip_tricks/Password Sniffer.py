...
pattern = re.compile(
    r"""(?P<found>(USER|USERNAME|PASS| 22 PASSWORD|LOGIN|BENUTZER|PASSWORT|AUTH| 23 ACCESS|ACCESS_?KEY|SESSION| 24 SESSION_?KEY|TOKEN)[=:\s].+)\b""",
...
def handle_packet(hdr, data):
    ...
    if not tcp_pkt.get_SYN() \
            and not tcp_pkt.get_RST() \
            and not tcp_pkt.get_FIN() \
            and match and match.groupdict()['found'] is not None:
        print("%s:%d -> %s:%d" % (
            ip_pkt.get_ip_src(), tcp_pkt.get_th_sport(), ip_pkt.get_ip_dst(), tcp_pkt.get_th_dport()))
